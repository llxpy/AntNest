...
@app.route("ws_preview")
def on_ws_preview(data):
    """工作空间文件预览：列出目录文件（目录优先，最多 120 项）。"""
    import antnest_runtime_state as _rs
    path = ((data or {}).get("path") or "").strip()
    if not path or not os.path.isdir(path):
        app.run_js(f"renderWsPreview({json.dumps(path or '')}, []);")
        return
    files = []
    try:
        names = sorted(os.listdir(path))
        dirs = [n for n in names if os.path.isdir(os.path.join(path, n))]
        fms = [n for n in names if not os.path.isdir(os.path.join(path, n))]
        for d in dirs[:40]:
            files.append({"name": d, "dir": True})
        for f in fms[:80]:
            files.append({"name": f, "dir": False})
    except Exception as e:
        bridge.trace("UI", "warn", f"ws_preview 失败: {e}")
    app.run_js(f"renderWsPreview({json.dumps(path)}, {json.dumps(files)});")
...
app.body(
    ...
    ui.div(cls="main")[
        # 工作空间栏（最左）
        ui.div(cls="workspace-col", id="workspace-col")[
            ui.div(cls="ws-head")[
                ui.h2()["工作空间"],
                ui.raw('<button class="ws-toggle-btn" id="ws-toggle-btn" onclick="toggleWorkspace()" title="折叠/展开工作空间">«</button>'),
            ],
            ...
            ui.raw('<div class="ws-preview" id="ws-preview" style="display:none">'
                   '<div class="ws-preview-title" id="ws-preview-title"></div>'
                   '<div class="ws-preview-files" id="ws-preview-files"></div>'
                   '<button class="btn ghost ws-preview-enter" onclick="wsPreviewEnter()">→ 进入此工作空间</button>'
                   '</div>'),
            ui.raw('<div class="ws-collapsed-bar" id="ws-collapsed-bar" onclick="expandWorkspace()" title="展开工作空间">工 作 空 间</div>'),
        ],
        # 左：聊天
        ui.div(cls="chat-col")[
            ui.div(cls="card chat")[
                ...
                ui.div(cls="chat-input-row")[
                    ...
                    ui.div(cls="chat-input-wrap")[
                        ...
                        ui.raw('<textarea class="chat-input" id="chat-input" rows="1" '
                               'placeholder="给蚁后下达任务…（Enter 发送，Shift+Enter 换行，可粘贴图片）" '
                               'oninput="phwCall(\'chat_input\',{value:this.value}); autoGrowInput(this)" '
                               'onkeydown="if(event.key===\'Enter\' && !event.shiftKey){ event.preventDefault(); onSend(); }" '
                               'onpaste="onChatPaste(event)"></textarea>'),
